...
void DetectorFactory::ReadClassList(std::string FileList){
   std::ifstream InFile(FileList.c_str());
    ...
    while(InFile >> Token >> LibName){
      // Modify LibName from NPLib name to Ganil2Root one
      LibName = LibName.replace(LibName.find("libNP"), std::string("libNP").length(), "libG2R");
...
////////////////////////////////////////////////////////////////////////////////
G2R::VDetector* DetectorFactory::Construct(std::string Token){
  ...
  if(m_Construct.find(Token)!=m_Construct.end())
   ...
   // Add absolute path to library name
    std::string path = getenv("GANIL2ROOT"); 
